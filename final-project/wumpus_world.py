...
def percept():
    ...
    if abs(x - wumpus[0]) + abs(y - wumpus[1]) <= 1:
        percepts.append('a stench')
    # Check if we are near the gold
    if (x, y) == gold and not hasGold:
        percepts.append('glittering')
    if wumpusKillFlag:
    ...
    if bumpFlag:
        ...
        bumpFlag = False
    print(
        f"You are in square ({x},{y}) and facing {headings[headingIndex]}. Your Score is {score}.")
    if percepts:
...
while playing:
    ...
    if action == 'C':  # Climb out of the cave
        if x == 1 and y == 1:
            ...
            print("There is no exit from the cave here!")
    elif action == 'L':  # turn left
        headingIndex = (headingIndex - 1) % 4
        print(f"Turned to face {headings[headingIndex]}")
        score = score - 1
        ...
        if (1 <= newcoord[0] <= xmax) and (1 <= newcoord[1] <= ymax):
            ...
            bumpFlag = True
            print(
                f"You cannot move further {headings[headingIndex]}.  Try another move.")
        score = score - 1
        ...
        if hasArrow:
            ...
            toWumpusMag = math.sqrt(toWumpus[0]**2 + toWumpus[1]**2)
